...
#define Range(b, ...) for([[maybe_unused]] auto b: range(__VA_ARGS__))
...
int main() {
	int n {in}, q {in};
	LL r {};
	vector<int> a = in.read(n), c(n + 1), p(n + 1);
	Range (i, q)
		c[(int) in - 1]++, c[in]--;
	for (int i {}, t {}; i < (int) size(c); i++)
		c[i] = (t += c[i]), p[i] = i;
	sort(begin(a), end(a));
	sort(begin(p), end(p), [&](int x, int y) {return c[x] < c[y];});
	Range (i, n)
		r += (LL) a[i] * c[p[i + 1]];
	outl(r);
}
