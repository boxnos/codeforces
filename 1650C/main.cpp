...
int main() {
	Test {
		...
		nth_element(begin(a), begin(a) + n * 2 - 1,end(a), [](T a, T b){return get<1>(a) < get<1>(b);});
		int r {accumulate(begin(a), begin(a) + n * 2, 0, [](int acc, T a) {return get<1>(a) + acc;})};
		sort(begin(a), begin(a) + n * 2);
		outl(r);
		Range (i, n)
			outl(get<2>(a[i]), ' ', get<2>(a[n * 2 - 1 - i]));
		outl("");
	}
}
