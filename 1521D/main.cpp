...
TAPP(char,"%c") TAPP(int,"%d") TAPP(LL,"%lld") TAPP(const char *, "%s")
#ifdef S_
...
int main() {
	Range (t, in) {
		...
		for (LL &i: b)
			s += i = in;
		sort(begin(b), end(b));
		if(binary_search(begin(b), end(b) - 1, (b1 = b.back(), b2 = s - b1 * 2)) ||
		   (binary_search(begin(b), end(b) - 2, (b1 = *(end(b) - 2), b2 = s - b1 * 2)) || b2 == b.back())) {
			for (LL i: b)
				if (i == b1)
					b1 = 0;
				else if (i == b2)
